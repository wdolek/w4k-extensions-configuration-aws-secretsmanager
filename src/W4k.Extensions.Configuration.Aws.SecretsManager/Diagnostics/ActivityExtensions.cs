...
public static class ActivityExtensions
{
    ...
    public static string FormatStopActivity(this Activity activity)
    {
        ...
        {
            (true, true) =>
                $"[{activity.StartTimeUtc:O}] {activity.Source.Name}:{activity.OperationName} Stopped, duration: {activity.Duration}; {evt.Name}; Failed: {err.Value}",
            (true, false) =>
                $"[{activity.StartTimeUtc:O}] {activity.Source.Name}:{activity.OperationName} Stopped, duration: {activity.Duration}; {evt.Name}",
